...
const Loader = () => (
  <div className="flex justify-center items-center p-4">
    ...
    <p className="ml-4 text-lg">Generando palabras...</p>
  </div>
);
...
const FlipCard: React.FC<FlipCardProps> = ({ player, onFlip }) => {
      ...
      <div className={`card-inner ${isFlipped ? "is-flipped" : ""}`}>
        <div className="card-front bg-slate-700 hover:bg-slate-600 flex flex-col justify-center items-center p-4 shadow-lg">
          <h3 className="text-2xl font-bold text-center">{player.name}</h3>
           <p className="mt-4 text-slate-300 text-center">
            {hasBeenRevealed ? 'Toca para ocultar/ver' : 'Toca para revelar'}
          </p>
        </div>
        <div className="card-back bg-slate-800 flex flex-col justify-center items-center p-4 shadow-lg border-2 border-cyan-400">
          {player.role === "impostor" ? (
            ...
            <>
              <p className="text-slate-300 text-center">La palabra es:</p>
...
const Timer: React.FC<TimerProps> = ({ seconds, onFinish }) => {
  ...
  return (
    <div className="text-6xl font-mono p-4 bg-slate-800 rounded-lg shadow-inner">
...
export function App() {
  ...
  const currentCategoryWords = useMemo(() => categoryWords[category] || [], [categoryWords, category]);
  ...
  const isSetupValid = useMemo(() => {
    ...
    return names.every(name => name.trim() !== '') && names.length > 0 && currentCategoryWords.length > 0;
  }, [playerNames, numPlayers, currentCategoryWords]);
  
  // --- API CALL ---
  const generateWordsForCategory = useCallback(async (cat: string) => {
    ...
    try {
      const schema = {
        ...
        properties: {
          items: {
            ...
            description: "A list of 50 well-known items from the specified category.",
          },
        },
        required: ["items"],
      };
      ...
      setCategoryWords(prev => ({ ...prev, [cat]: ['Fallback 1', 'Fallback 2'] }));
    } finally {
  ...
  const handleStartGame = () => {
    ...
    const word = currentCategoryWords[Math.floor(Math.random() * currentCategoryWords.length)];
  ...
  const renderReveal = () => (
    <div className="w-full max-w-4xl mx-auto p-8 text-center">
        ...
        <p className="text-slate-300 mb-8">Cada jugador debe tocar su carta para ver su rol. ¡No se la muestres a nadie!</p>
        <div className="flex flex-wrap justify-center gap-6">
            {players.map(p => <FlipCard key={p.name} player={p} onFlip={() => setFlippedCards(c => c + 1)} />)}
        </div>
        {flippedCards === players.length && (
  ...
  return (
    <main className="min-h-screen w-full flex flex-col items-center justify-center p-4 bg-gradient-to-br from-slate-900 to-indigo-900">
        <h1 className="text-6xl font-bold text-center text-white mb-8 drop-shadow-lg" style={{ textShadow: '0 0 15px rgba(0, 255, 255, 0.6)' }}>
            EL IMPOSTOR
        </h1>
